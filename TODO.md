...
- [x] **E. Mailing — historique + file globale** (polish) — **Fait (08-04)** : actions déjà en **icônes** (réutiliser/voir la file/archiver/restaurer/supprimer) ; **« Supprimer » sécurisé** (`@admin_required`, visible seulement dans les archivées, Archiver en avant, confirmation renforcée) — c'était déjà en place. Polish appliqué : badges unifiés (canoniques `badge-green/danger/gray` + `badge-bounce`/`badge-unsub`), `<style>` inline sorti dans `style.css`, `btn-sm→btn-small`. *(Reste optionnel : câbler le tri client des colonnes `data-sort` de l'historique, aujourd'hui inertes.)*
