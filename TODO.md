...
- [x] **Point d'entrée unique des migrations `tools/migrate.py`** (runner ledger `schema_migrations`, registre ordonné des 21 scripts, `--dry-run`/`--stamp`/`--safe-only`) — **Fait (05/08, dans l'image v2.0.0)**. À chaque changement de schéma : ajouter une ligne dans `MIGRATIONS`. Après déploiement : `docker compose exec app python tools/migrate.py`. *(Alembic reporté au jour Postgres.)*
