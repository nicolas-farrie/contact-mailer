...
@app.route('/formulaires/new', methods=['GET', 'POST'])
@login_required
def formulaire_new():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('formulaire_detail', id=pf.id))
    return render_template('formulaire_edit.html', form=None, listes=listes)


@app.route('/formulaires/<int:id>', methods=['GET'])
@login_required
def formulaire_detail(id):
...
@app.route('/formulaires/<int:id>/edit', methods=['GET', 'POST'])
@login_required
def formulaire_edit(id):
    pf = PreferenceForm.query.get_or_404(id)
    listes = Liste.query.order_by(Liste.nom).all()
    if request.method == 'POST':
        nom = request.form.get('nom', '').strip()
...
@app.route('/formulaires/<int:id>/delete', methods=['POST'])
@login_required
def formulaire_delete(id):
    pf = PreferenceForm.query.get_or_404(id)
    nom = pf.nom
    db.session.delete(pf)
    db.session.commit()
    flash(f'Formulaire "{nom}" supprimé.', 'success')
    return redirect(url_for('formulaires'))
...
@app.route('/p/<form_token>/<contact_uid>', methods=['GET', 'POST'])
def preferences_public(form_token, contact_uid):
