#!/usr/bin/env python3
"""
Migration : ajout des champs de suppression douce (corbeille) aux contacts.

Usage :
    python tools/migrate_add_softdelete.py                    # migration réelle
    python tools/migrate_add_softdelete.py --dry-run          # simulation sans modification
    python tools/migrate_add_softdelete.py --db data/other.db # base personnalisée

Le script :
1. Crée un backup automatique de la base
2. ALTER TABLE contact ADD COLUMN is_deleted BOOLEAN NOT NULL DEFAULT 0
3. ALTER TABLE contact ADD COLUMN deleted_at DATETIME
4. ALTER TABLE contact ADD COLUMN deleted_by_id INTEGER
"""
import sqlite3
import shutil
import sys
...
def migrate(db_path, dry_run=False):
    ...
    if 'deleted_by_id' not in columns:
        to_add.append(('deleted_by_id', 'INTEGER'))

    if not to_add:
        print("Les colonnes de corbeille existent déjà. Migration non nécessaire.")
        conn.close()
        return True
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Migration : ajout champs corbeille contacts')
